...
- [x] `goplt init` — interactively scaffold a new `template.toml` for template authors
...
- [x] Add a command to register a template repository and the option `--remote` to `goplt list` that will update the cache of the registered templates.

## Variable system

- [x] `KindChoiceInt` / numeric variables — integer values (e.g. `port = 8080`) currently produce a type error
...
- [ ] Add [goscaf](https://github.com/iyashjayesh/goscaf) templates.
...
- [ ] `go-service-grpc` domain — meta-template skeleton for gRPC services
- [ ] `go-service-http` domain — meta-template skeleton for gRPC services
